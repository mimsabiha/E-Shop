...
export const registerBankUser=(user)=>async dispatch=>{
    ...
    try{
        const response=await axios.post('http://localhost:4000/bankAPI/users/register', user)
...
export const findBankUser=(user)=>async dispatch=>{
    ...
    try{
        ...
        const response=await axios.get('http://localhost:4000/bankAPI/users/getUID', {params: {user: user}})
