...
export const verifyAdmin=(admin)=>async dispatch=>{
    ...
    try{
        const response=await axios.post('http://localhost:8000/storeAPI/admin/verifyAdmin', admin)
...
export const getAllOrders=()=>async dispatch=>{
    ...
    try {
        const response = await axios.get('http://localhost:8000/storeAPI/orders/getAllOrders')
...
export const verifyAOrder=(orderid)=>async dispatch=>{
    ...
    try {
        ...
        const response = await axios.post('http://localhost:8000/storeAPI/orders/verifyOrder',orderid);
...
export const updateAdminBalance =(email,amount) => async dispatch=>{
    ...
    try {
        ...
        const accUpdate = await axios.post('http://localhost:4000/bankAPI/users/updateAdminBalance',{email,amount})
...
export const updateBalance =(email,amount,token) => async dispatch=>{
    ...
    try {
        // console.log("adminAction Up ",email,amount );
        const accUpdate = await axios.post('http://localhost:4000/bankAPI/users/updateBalance',{email,amount,token})
